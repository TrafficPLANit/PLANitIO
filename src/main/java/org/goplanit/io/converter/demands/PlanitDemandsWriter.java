...
public class PlanitDemandsWriter extends PlanitWriterImpl<Demands> implements DemandsWriter {
  ...
  private void populateXmlUserClasses(Demands demands, final XMLElementDemandConfiguration xmlDemandConfiguration) {
    ...
    demands.userClasses.streamSortedBy(getPrimaryIdMapper().getUserClassIdMapper()).forEach( userClass -> {
      ...
      if(userClass.getMode()==null) {
        ...
        xmlUserClass.setTravellertyperef(
                getPrimaryIdMapper().getTravellerTypeIdMapper().apply(userClass.getTravelerType()));
      }      
   ...
   * @param xmlDemandConfiguration to use
   */
  private void populateXmlTravellerTypes(
          final Demands demands, final XMLElementDemandConfiguration xmlDemandConfiguration) {
    ...
    demands.travelerTypes.streamSortedBy(getPrimaryIdMapper().getTravellerTypeIdMapper()).forEach(travellerType -> {
      ...
      xmlTravellerTypes.getTravellertypes().add(xmlTravellerType);
  ...
  private double populateXmlOdRowMatrix(
    ...
    this.referenceZoning.getOdZones().streamSortedBy(zoneIdMapper).forEach( originZone -> {
      ...
      /* odrow */
      var xmlOdRow = new org.goplanit.xml.generated.v2.Odrow();
  ...
  private void populateXmlOdDemands(Demands demands) {
    ...
    demands.timePeriods.streamSortedBy(getPrimaryIdMapper().getTimePeriodIdMapper()).forEach( timePeriod -> {
      ...
      modes.stream().sorted(Comparator.comparing(
        ...
        if(odDemandsEntry != null) {
          ...
          xmlOdDemands.getOdcellbycellmatrixesAndOdrowmatrixesAndOdrawmatrixes().add(xmlOdDemandEntryMatrix);
          ...
          LOGGER.info(String.format("OD demands matrix: total trips %.2f (veh/h)  %.2f pcu factor , " +
                  "timePeriod: %s, user-class %s",
                  odDemandVehH, userClass.getMode().getPcu(), timePeriod.toString(), userClass.toString()));
        }
      });
    });
  }
   ...
   */
  protected PlanitDemandsWriter(
          final PlanitDemandsWriterSettings settings, final XMLElementMacroscopicDemand xmlRawDemands) {
   ...
   */
  protected PlanitDemandsWriter(
          final PlanitDemandsWriterSettings settings,
