 ...
 */
  ...
  */
    public XmlMacroscopicLinkSegmentType(String name, double capacityPerLane, double maximumDensityPerLane, int linkType) {
        this.name = name;
 ...
 /**
  * Create or update an XmlMacroscopicLinkSegmentType object using the input from the current row in the XML input file
  * 
  ...
  * @return                                                   XmlMacroscopicLinkSegmentType object, created or updated to include data from current row in the XML file
  */
    public static XmlMacroscopicLinkSegmentType createOrUpdateLinkSegmentType(String name, 
        ...
        if (!existingLinks.containsKey(linkType)) {
            ...
            if (capacityPerLane != linkSegmentType.getCapacityPerLane()) {
                LOGGER.warning("Different capacity per lane values for Link Type " + linkSegmentType.getName() + ".  Will use the highest one.");
            }
            if (capacityPerLane > linkSegmentType.getCapacityPerLane()) {
                linkSegmentType.setCapacityPerLane(capacityPerLane);                
            }
            if (maximumDensityPerLane != linkSegmentType.getMaximumDensityPerLane()) {
                LOGGER.warning("Different maximum density per lane values for link type " + linkSegmentType.getName() + ".  Will use the highest one.");
            }
            if (maximumDensityPerLane > linkSegmentType.getMaximumDensityPerLane()) {
        ...
        if (modeNo == 0) {
            modeMap.keySet().forEach(eachModeNo -> {updateLinkSegmentType(linkSegmentType, modeMap, eachModeNo, speed,linkType);});
        } else {
 ...
 * 
 * @param linkSegmentType                       XmlMacroscopicLinkSegmentType linkSegmentType object to be updated
 * @param modeMap                                  Map storing modes
 * @param modeNo                                     number of the mode used in the XML input file
 * @param speed                                         maximum speed
 * @param linkType                                     id number of the link type
 */
    private static void updateLinkSegmentType(XmlMacroscopicLinkSegmentType linkSegmentType, 
                                                                              Map<Integer, Mode> modeMap,
                                                                              int modeNo, 
                                                                              double speed, 
                                                                              int linkType) {
        long modeId = modeMap.get(modeNo).getId();
        if (speed < 0.0) {
            LOGGER.warning("A negative maximum speed has been defined for Link Type " + linkSegmentType.getName() + " and Mode " + modeMap.get(modeNo).getName() + ".  Setting the speed to zero instead (which means vehicles of this type are forbidden in links of this type.)");
            speed = 0.0;
        }
        linkSegmentType.getSpeedMap().put(modeId, speed);
