...
/**
 * This class contains methods to update the Demands object using input values from the XML demands input file.
 * 
 * @author gman6028
 *
 */
...
/**
  * Creates a MatrixDemand object from a List of Odmatrix objects read in from the XML input file and registers it in the Demands object
  * 
  * @param demands                                              the PlanIt Demands object to be populated
  * @param oddemands                                          List of generated Odmatrix objects with data from the XML input file
  ...
  */
    public static void createAndRegisterDemandMatrix(Demands demands, 
																				            List<Odmatrix> oddemands, 
																				            Map<Integer, Mode> modeMap, 
																				            Map<Integer, TimePeriod> timePeriodMap, 
																				            int noCentroids,
																				            Zones zones) throws Exception {
		Map<Mode, Map<TimePeriod, MatrixDemand>> demandsPerTimePeriodAndMode = initializeDemandsPerTimePeriodAndMode(timePeriodMap, modeMap, noCentroids); 
 ...
 */
    private static Map<Mode, Map<TimePeriod, MatrixDemand>> initializeDemandsPerTimePeriodAndMode(Map<Integer, TimePeriod> timePeriodMap, Map<Integer, Mode> modeMap, int noCentroids) throws PlanItException {
 ...
 * @throws Exception              thrown if there is an error during processing
 */
    private static void updateDemandMatrixFromOdMatrix(Odmatrix odmatrix, double pcu, MatrixDemand matrixDemand, Zones zones) throws Exception {
 ...
 * @param zones                           zones in the current network
 */
    private static void updateDemandMatrixFromCellByCellMatrix(Odcellbycellmatrix odcellbycellmatrix, double pcu, MatrixDemand matrixDemand, Zones zones) {
        ...
        for (Odcellbycellmatrix.O originZone : o) {
        	...
        	for (Odcellbycellmatrix.O.D demandZone : d) {
        		updateMatrixDemand(matrixDemand, originZone.getRef().intValue(), demandZone.getRef().intValue(), pcu, demandZone.getValue(), zones);
        	}
        }
    }
 ...
 * @param zones                      zones in the current network
 */
    private static void updateDemandMatrixFromOdRowMatrix(Odrowmatrix odrowmatrix, double pcu, MatrixDemand matrixDemand, Zones zones) {
        ...
        for (Odrowmatrix.Odrow originZone : odrow) {
         	...
         	for (int i=0; i<rowValuesAsString.length ; i++) {
         		updateMatrixDemand(matrixDemand, originZone.getRef().intValue(), i+1, pcu, Double.parseDouble(rowValuesAsString[i]), zones);
         	}
        }
    }
 ...
 */
    private static void updateDemandMatrixFromOdRawMatrix(Odrawmatrix odrawmatrix, double pcu, MatrixDemand matrixDemand, Zones zones) throws Exception {
        ...
        if (originSeparator.equals(destinationSeparator)) {
        	 ...
        	 updateDemandMatrixForDifferentSeparators(values, originSeparator, destinationSeparator, pcu, matrixDemand, zones);
        }
    }
 ...
 * @throws Exception                    thrown if the Odrawmatrix cannot be parsed into a square matrix
 */
    private static void updateDemandMatrixForDifferentSeparators(Values values, String originSeparator, String destinationSeparator, double pcu, MatrixDemand matrixDemand, Zones zones) throws Exception {
    	...
    	for (int i=0; i <noRows; i++) {	
    		...
    		for (int col=0; col < noCols; col++) {
    			updateMatrixDemand(matrixDemand, i+1, col+1, pcu, Double.parseDouble(allValuesAsString[col]), zones);
    		}
    	}
    }
 ...
 * @throws Exception                    thrown if the Odrawmatrix cannot be parsed into a square matrix
 */
    private static void updateDemandMatrixForEqualSeparators(Values values, String separator, double pcu, MatrixDemand matrixDemand, Zones zones) throws Exception {
    	...
    	for (int i=0; i <noRows; i++) {
    		int row = i * noRows;
    		for (int col=0; col < noCols; col++) {
    			updateMatrixDemand(matrixDemand, i+1, col+1, pcu, Double.parseDouble(allValuesAsString[row + col]), zones);
    		}
    	}
    }
 ...
 * @param zones                      zones in the current network
 */
    private static void updateMatrixDemand(MatrixDemand matrixDemand, int rowRef, int colRef, double pcu, double demandValue, Zones zones) {
