...
/**
 * JUnit test cases for the converters provided in the PLANit native format that auto-convert from
 * (unqualified) v1 version to v2 version
 * 
 * @author markr
 *
 */
public class V1ToV2LegacyConverterTest extends TestBase {
  ...
  public static void setUp() throws Exception {
    if (LOGGER == null) {
      LOGGER = Logging.createLogger(V1ToV2LegacyConverterTest.class);
    } 
  }
  ...
  public void testPlanit2PlanitNetworkConverter() {
    try {
      final String projectPath = Path.of(TEST_CASE_PATH.toString(),"v1v2converter_test").toString();
      final String inputPath = Path.of(projectPath, "v1input").toString();
      final String referencePath = Path.of(projectPath, "v2refoutput").toString();
      ...
      org.hamcrest.MatcherAssert.assertThat(
          ...
          CompareMatcher.isSimilarTo(Input.fromFile(Path.of(referencePath,"network.xml").toString())));
  ...
  public void testPlanit2PlanitIntermodalNoServicesConverter() {
    try {
      ...
      org.hamcrest.MatcherAssert.assertThat(
          ...
          CompareMatcher.isSimilarTo(Input.fromFile(Path.of(referencePath,"zoning.xml").toString())));
  ...
  /**
   * Test that reading a planit intermodal network (network and (pt) zoning) in native format and then
   * writing it results in the same
   * files as the original input that was read.
   */
  @Test
  public void testPlanit2PlanitIntermodalServicesConverter() {
    try {
      ...
      org.hamcrest.MatcherAssert.assertThat(
          ...
          CompareMatcher.isSimilarTo(Input.fromFile(Path.of(referencePath,"service_network.xml").toString())));

      /* use non-deprecated hamcrest version instead of junit for comparing */
      org.hamcrest.MatcherAssert.assertThat(
          /* xml unit functionality comparing the two files */
          Input.fromFile(Path.of(projectPath,"routed_services.xml").toString()),
          CompareMatcher.isSimilarTo(Input.fromFile(Path.of(referencePath,"routed_services.xml").toString())));
